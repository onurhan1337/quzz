...
export class TraceCollector {
   ...
   */
  initialize(outputPath?: string, autoSave: boolean = false, devMode: boolean = false): void {
  ...
  addTrace(metadata: TraceMetadata): void {
    ...
    if (trace.duration && (!this.session!.slowestComponent || trace.duration > this.session!.slowestComponent.duration)) {
  ...
  updateTrace(traceId: string, updates: Partial<TraceMetadata>): void {
    ...
    if (trace) {
      ...
      if (updates.wallClockTime !== undefined) trace.wallClockTime = updates.wallClockTime
      ...
      if (trace.duration && this.session) {
        if (!this.session.slowestComponent || trace.duration > this.session.slowestComponent.duration) {
          this.session.slowestComponent = {
            name: trace.componentName,
            duration: trace.duration,
          }
  ...
  getTracesByTimeRange(startTime: number, endTime: number): CollectedTrace[] {
    return this.rootTraces.filter(
      trace => trace.startTime >= startTime && (trace.endTime || trace.startTime) <= endTime
    )
  }
