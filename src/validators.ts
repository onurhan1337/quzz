...
export class ConfigValidator {
  ...
  static validate(config: Partial<QuzzConfig>): ValidationResult {
    ...
    if (config.maxPropDepth !== undefined) {
      if (config.maxPropDepth < 0 || config.maxPropDepth > this.MAX_PROP_DEPTH_LIMIT) {
        errors.push(`maxPropDepth must be between 0 and ${this.MAX_PROP_DEPTH_LIMIT}`)
      }
    }

    if (config.maxStringLength !== undefined) {
      if (config.maxStringLength < 0 || config.maxStringLength > this.MAX_STRING_LENGTH_LIMIT) {
        errors.push(`maxStringLength must be between 0 and ${this.MAX_STRING_LENGTH_LIMIT}`)
      }
    }
    ...
    if (config.performance?.warnThreshold !== undefined) {
      ...
      if (threshold < 0 || threshold > this.WARN_THRESHOLD_LIMIT) {
        errors.push(`performance.warnThreshold must be between 0 and ${this.WARN_THRESHOLD_LIMIT}`)
      }
      if (threshold > 10000) {
        warnings.push('performance.warnThreshold > 10s may not trigger warnings effectively')
      }
    }
...
export class ContextValidator {
  ...
  static validateContextState(
    ...
    if (traceStack.length > this.MAX_TRACE_STACK_SIZE) {
      errors.push(`Trace stack exceeds maximum size (${this.MAX_TRACE_STACK_SIZE})`)
    }

    if (traceMap.size > this.MAX_TRACE_MAP_SIZE) {
      warnings.push(`Trace map is large (${traceMap.size} entries), consider cleanup`)
    }

    const depth = traceStack.length
    if (depth > this.MAX_CONTEXT_DEPTH) {
      errors.push(`Context nesting depth exceeds limit (${this.MAX_CONTEXT_DEPTH})`)
    }
...
export class NodeVersionValidator {
  private static readonly MIN_NODE_VERSION = '14.0.0'
  ...
  static checkCompatibility(): ValidationResult {
    ...
    if (typeof process === 'undefined' || !process.versions?.node) {
      warnings.push('Not running in Node.js environment, AsyncLocalStorage unavailable')
    ...
    if (major < 14) {
      errors.push(`Node.js version ${nodeVersion} is not supported. Minimum required: 14.0.0`)
    } else if (major === 14 && minor < 18) {
      warnings.push(`Node.js ${nodeVersion} has limited AsyncLocalStorage support. Consider upgrading to 14.18+ or 16+`)
    }

    if (major < 16) {
      warnings.push('Node.js 16+ recommended for optimal AsyncLocalStorage performance')
    }
  ...
  static checkForLeaks(): ValidationResult {
    ...
    for (const [contextId, createdAt] of this.contextCreationTimes) {
      ...
      if (age > this.CONTEXT_LIFETIME_WARNING) {
        warnings.push(`Context ${contextId} has been alive for ${Math.round(age / 1000)}s`)
      }
    }

    if (this.contextCreationTimes.size > 500) {
      warnings.push(`High number of tracked contexts (${this.contextCreationTimes.size})`)
    }
